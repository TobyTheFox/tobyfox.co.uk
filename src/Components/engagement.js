...
class Engagement extends Component {
  ...
  handleSubmit = async (event) => {
    ...
    const formData = {
      ...
      notes: this.state.dietary,
      pizza: this.state.pizzaChoice,
      isSpam: this.state.otherField && this.state.otherField.trim() !== ''
    };

    const scriptURL = "https://script.google.com/macros/s/AKfycbwPEmmgkZgRcQk3XFUwYhi4U1xkdEc3Afz1kcefP_0ZmA625Tfomny8atEwV9yvRgS_/exec";
    ...
    try {
      ...
      if (result.success) {
        ...
        this.setState({ submitted: true, showModal: true, showConfetti: isAttending, isSubmitting: false, submitError: '' });
        ...
        const message = result.error || 'Something went wrong while sending your RSVP. Please try again.';
        console.error('Error submitting RSVP:', message);
        this.setState({ isSubmitting: false, submitError: message });
      }
    } catch (error) {
      const message = error && error.message ? error.message : 'Something went wrong while sending your RSVP. Please try again.';
  ...
  render() {
    const { guestName, attending, dietary, pizzaChoice, otherField, submitted, showModal, showConfetti, isSubmitting, submitError } = this.state;
    const sectionLinks = [
      { id: 'when', label: 'When?' },
      { id: 'where', label: 'Where?' },
      { id: 'dress-code', label: 'Dress' },
      { id: 'what-to-expect', label: 'What?' },
    ...
    return (
      <div className="rsvp-body-wrapper">
        ...
        <div className="rsvp-card">
          {showConfetti && (
            <div className="rsvp-confetti" aria-hidden="true">
              {Array.from({ length: 120 }).map((_, index) => {
                ...
                const duration = 3.6 + (index % 7) * 0.2;
          ...
          <h2 className="rsvp-subtitle">...would like to invite you to our...</h2>
          ...
          {!submitted ? (
            <form className="rsvp-form" onSubmit={this.handleSubmit}>
              ...
              {attending && (
                <div className="rsvp-form-group">
                  <label className="rsvp-label">Favorite pizza?</label>
              ...
              <div className="rsvp-form-group" style={{ display: 'none' }}>
                <label className="rsvp-label" htmlFor="otherField">Other</label>
                <input
                  type="text"
                  id="otherField"
                  name="otherField"
              ...
              <button type="submit" className={`rsvp-submit-btn ${isSubmitting ? 'is-loading' : ''}`} disabled={isSubmitting} aria-busy={isSubmitting}>
                {isSubmitting ? (
                  <span className="rsvp-submit-loading">
